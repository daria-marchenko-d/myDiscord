...
void update_channel_list(const char **channels, int num_channels)
{
    ...
    gtk_widget_set_visible(list_box_channels, TRUE);
}
...
{
    const char *message = gtk_editable_get_text(GTK_EDITABLE(entry_message));
...
GtkWidget *create_emoji_selector(GtkWidget *parent_widget, GtkApplicationWindow *window, const char *message_id)
{
    ...
    // GMenu для GtkPopoverMenu
    GMenu *menu = g_menu_new();

    for (int i = 0; i < num_emojis; i++)
    {
        char action_name[64];
        snprintf(action_name, sizeof(action_name), "reaction_%d", i);

        // Додаємо пункт меню, який викликає відповідну дію
        g_menu_append(menu, emojis[i], g_strdup_printf("win.%s", action_name));

        // Створюємо GSimpleAction
        GSimpleAction *action = g_simple_action_new(action_name, NULL);

        // Зберігаємо поточний смайлик і message_id у дії (через структуру)
        struct
        {
            const char *emoji;
        ...
        data->emoji = emojis[i];
        data->message_id = g_strdup(message_id); // Копія, бо може бути втрачене

        g_signal_connect_data(action, "activate", G_CALLBACK(send_reaction_request),
                              data, (GClosureNotify)g_free, 0);

        // Реєструємо дію на вікні
        g_action_map_add_action(G_ACTION_MAP(window), G_ACTION(action));
    }

    // Створюємо GtkPopoverMenu
    GtkWidget *popover = gtk_popover_menu_new_from_model(G_MENU_MODEL(menu));
    gtk_widget_set_parent(popover, parent_widget); // до чого прикріпити
...
{
    GtkWidget *container = gtk_widget_get_first_child(GTK_WIDGET(text_view_chat));
    while (container)
    {
        ...
        while (child)
        {
            if (gtk_widget_get_name(child) &&
                strcmp(gtk_widget_get_name(child), "reactions_box") == 0)
            {
                char buf[64];
                snprintf(buf, sizeof(buf), "%s %d", emoji, count);
                GtkWidget *reaction_label = gtk_label_new(buf);
                gtk_box_append(GTK_BOX(child), reaction_label);
                gtk_widget_set_visible(reaction_label, TRUE);
